...
// Alternative raw I2C IMU variables
bool rawIMUAvailable = false;
...
void setup() {
  ...
  // If standard IMU init failed, try raw I2C approach
  if (!imuAvailable) {
    USBSerial.println("Standard IMU init failed, trying raw I2C...");
    initRawIMU();
  }
...
void drawSpeedDisplay() {
  if (gps.location.isValid() && gps.speed.isValid()) {
    ...
    if (!lastHadFix || displaySpeed != lastSpeed) {
      ...
      if (imuAvailable || rawIMUAvailable) {
        // Draw IMU status and simple visualization
        gfx->setTextColor(CYAN);
        gfx->setTextSize(1);
        gfx->setCursor(10, 50);
        gfx->print(rawIMUAvailable ? "RAW:" : "IMU:");
...
// Alternative raw I2C IMU functions
void initRawIMU() {
  USBSerial.println("Trying raw I2C IMU communication...");
  
  // Try to read WHO_AM_I register from common addresses
  Wire.beginTransmission(0x6B);
  Wire.write(0x00);  // WHO_AM_I register for QMI8658
  if (Wire.endTransmission() == 0) {
    Wire.requestFrom(0x6B, 1);
    if (Wire.available()) {
      uint8_t whoami = Wire.read();
      USBSerial.printf("WHO_AM_I at 0x00: 0x%02X\n", whoami);
    }
  }
    ...
    if (Wire.endTransmission() == 0) {
      ...
      if (Wire.available()) {
        ...
        if (value != 0x00 && value != 0xFF) {
          USBSerial.printf("Register 0x%02X: 0x%02X\n", reg, value);
        }
      }
    }
  }
  
  rawIMUAvailable = true;
  USBSerial.println("Raw IMU initialized");
}

void readRawIMU() {
  if (!rawIMUAvailable) return;
  
  // Try to read accelerometer data from common register addresses
  // This is a generic approach that might work with various IMU chips
  
  // Try QMI8658 style registers first
  Wire.beginTransmission(0x6B);
  Wire.write(0x35);  // QMI8658 accel X low byte
  if (Wire.endTransmission() == 0) {
    ...
    if (Wire.available() >= 6) {
      ...
      // Convert to g (assuming ±4g range)
      rawAccelX = ax / 8192.0;
      rawAccelY = ay / 8192.0;
      rawAccelZ = az / 8192.0;
      return;
  ...
  // If that didn't work, try LSM6DS3 style registers
  Wire.beginTransmission(0x6B);
  Wire.write(0x28);  // LSM6DS3 accel X low byte
  if (Wire.endTransmission() == 0) {
    Wire.requestFrom(0x6B, 6);
    if (Wire.available() >= 6) {
      int16_t ax = Wire.read() | (Wire.read() << 8);
